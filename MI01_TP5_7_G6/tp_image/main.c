...
const gchar default_image[] = "Suivez-moi.bmp";
...
void run_processing_task(const gchar *target, int showtime)
{
    ...
    clock_t start = clock();
    for (int i = 0; i < (showtime ? process_repetitions:1); ++i) {
...
/* compare_images
 *
 * Affiche le taux de ressemblance entre les images
 *
 */
void compare_images()
{
    ...
    for(int i = 1; i < N_BITMAPS; i++) {
        ...
        /* Lancer l'implémentation de référence en C */
        run_processing_task("c", 0);

        /* Récupérer le buffer à comparer */
        GdkPixbuf *image_c = gdk_pixbuf_copy(bitmaps[i].pixbuf);
        uint32_t *pixels_c = (uint32_t *) gdk_pixbuf_get_pixels(image_c);

        /* Lancer l'implémentation ASM et comparer */
        run_processing_task("asm", 0);
        uint32_t *pixels_cmp = (uint32_t *) gdk_pixbuf_get_pixels(bitmaps[i].pixbuf);
        for (int pixel = 0; pixel < n; pixel++) {
            diff_c_asm += pixels_cmp[pixel] == pixels_c[pixel] ? 0 : 1;
        }

        /* Lancer l'implémentation SIMD et comparer */
        run_processing_task("simd", 0);
        pixels_cmp = (uint32_t *) gdk_pixbuf_get_pixels(bitmaps[i].pixbuf);
        for (int pixel = 0; pixel < n; pixel++) {
            diff_c_simd += pixels_cmp[pixel] == pixels_c[pixel] ? 0 : 1;
        }

        message_dialog("Comparaison", GTK_MESSAGE_INFO,
            "%s : \n\n"
            "Pixels comparés : %d\n\n"
            "Différence C-ASM\n\n"
            "Pixels différents : %d\n\n"
            "Taux de correspondance : %.3f\%\n\n"
            "Différence C-SIMD\n\n"
            "Pixels différents : %d\n\n"
            "Taux de correspondance %.3f\%",
            bitmaps[i].name, n,
            diff_c_asm, (n - diff_c_asm) * 100.0 / n,
            diff_c_simd, (n - diff_c_simd) * 100.0 / n);
        g_object_unref(image_c);
    }
}
...
static const gchar ui_info[] =
  ...
  "        </item>"
  "        <item>"
  "          <attribute name='label'>_Comparer les images</attribute>"
  "          <attribute name='action'>app.compare</attribute>"
  "        </item>"
  "      </section>"
  "      <section>"
  "        <item>"
 ...
 *
 */
static void activate_compare(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    compare_images();
}
...
static GActionEntry app_entries[] = {
    ...
    { "compare", activate_compare, NULL, NULL, NULL }
};
